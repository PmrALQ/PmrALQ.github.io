    ...
    <div class="mx-auto max-w-3xl">
      ...
      <header class="mb-10">
        <div class="flex items-center gap-3 mb-4 flex-wrap">
          <span
            v-if="page?.type"
            class="inline-flex items-center rounded-full bg-primary-50 px-3 py-1 text-xs font-medium text-primary-600 dark:bg-primary-900/30 dark:text-primary-400"
          >
            {{ page.type }}
          </span>
          <time v-if="page?.date" class="text-sm text-gray-500 dark:text-gray-400">
        ...
        <p v-if="page?.description" class="text-lg text-gray-500 dark:text-gray-400">
          {{ page.description }}
        </p>
      </header>
      ...
      <div v-if="page?.demoUrl" class="mb-12 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-lg">
        ...
        <div class="flex items-center justify-between px-5 py-3 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-medium text-gray-500 dark:text-gray-400 ml-3">
              {{ t('projects.viewDemo') }}
            </span>
          </div>
          <a
      ...
      <div v-if="page?.tech?.length" class="mb-8">
        ...
        <div class="flex flex-wrap gap-2">
          <span
            ...
            :key="t"
            class="inline-flex items-center rounded-full bg-primary-50 px-3 py-1 text-sm font-medium text-primary-700 dark:bg-primary-900/20 dark:text-primary-300"
          >
            {{ t }}
          </span>
        </div>
      ...
      <div v-if="page?.sourceUrl" class="mb-10">
        <a
          ...
          class="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-5 py-3 text-sm font-medium text-gray-700 hover:text-primary-600 hover:border-primary-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:text-primary-400 dark:hover:border-primary-700 transition-all shadow-sm"
        >
...
useHead({
  ...
  meta: [
    { name: 'description', content: computed(() => page.value?.description || t('projects.description')) },
  ],
})
</script>
